...
for((i=1;i<=65;i++)); do ./$cfg -s vendor_confile_qid -a $i -p $i -d ":::::"; done
echo "" && echo "Change delimiter(s) ..."
./$cfg -s vendor_confile_qid -o "---" -d ":::::"
echo "" && echo "To get the tokens from list ..."
for((i=1;i<=65;i++)); do ./$cfg -s vendor_confile_qid -i $i -d "---"; done
echo "" && echo "Change delimiter(s) ..."
./$cfg -s vendor_confile_qid -o "^_^" -d "---"
echo "" && echo "Reverse order the list for one by one from the ending	 ..."
for((i=1;i<=65;i++)); do ./$cfg -s vendor_confile_qid -m $i -p $(echo `expr 66 - $i`) -d "^_^"; done
echo "" && echo "Change delimiter(s) ..."
./$cfg -s vendor_confile_qid -o "-_-||" -d "^_^"
echo "" && echo "Reverse order the list for one by one again from the beginning ..."
for((i=1;i<=65;i++)); do ./$cfg -s vendor_confile_qid -m $i -p $i -d "-_-||"; done
echo "" && echo "Change delimiter(s) ..."
./$cfg -s vendor_confile_qid -o "@_@" -d "-_-||"
...
./$cfg -s tmp_nvram -o "---" -d "&&"
./$cfg -s tmp_nvram -o "^^^" -d "---"
./$cfg -s tmp_nvram -o "~v~" -d "^^^"
./$cfg -s tmp_nvram -o " " -d "~v~"
./$cfg -s tmp_nvram -o "----" -d " "
./$cfg -s tmp_nvram -o "{[]}" -d "----"
./$cfg -s tmp_nvram -o "~*~" -d "{[]}"
./$cfg -s tmp_nvram -o "~*~" -d "~*~"
