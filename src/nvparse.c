 ...
 *
 * $Id: nvparse.c,v 1.1 2007/04/17 10:12:30 timo Exp $
 */
...
#include <dni_netconf.h>
#include <dni_bcmnvram.h>
#include <dni_shutils.h>
#include <dni_nvparse.h>
#include <dni_bcmconfig.h>
//#include <dni_dummy.h>
...
//modified by kane from BELKIN_ROUTER to STANDARD_PRODUCT_ONLY
#ifdef	STANDARD_PRODUCT_ONLY
bool
...
#endif //#ifdef		STANDARD_PRODUCT_ONLY
//end modified
...
valid_forward_port(const netconf_nat_t *nat)
{
	...
	// Modified by wayne.lu on 8/30/2004
	// The constraint of inbound ports same as internal ports is released.
	// Check port range size
	/*
...
get_forward_port(int which, netconf_nat_t *nat)
{
	...
	if (!strncasecmp(proto, "tcp", 3))
	{
		...
		// Added by wayne.lu on 9/17/2004
		nat->strChain = CHAIN_TCP_PREROUTE;
	}
	else if (!strncasecmp(proto, "udp", 3))
	{
		nat->match.ipproto = IPPROTO_UDP;
		// Added by wayne.lu on 9/17/2004
		nat->strChain = CHAIN_UDP_PREROUTE;
	}
	else
		return FALSE;
...
valid_filter_client(const netconf_filter_t *start, const netconf_filter_t *end)
{
	...
	/* Check protocol */
	// Modified by wayne.lu on 7/26/2004
	// Since protocol can be both, the following code are revised 
	/*
	...
	// End of modification
	if ((start->match.ipproto != IPPROTO_TCP && start->match.ipproto != IPPROTO_UDP) || 
	   (end->match.ipproto != IPPROTO_TCP && end->match.ipproto != IPPROTO_UDP) )
		return FALSE;
...
get_filter_client(int which, netconf_filter_t *start, netconf_filter_t *end)
{
	...
	else if (!strncasecmp(proto, "both", 4))
	{
		start->match.ipproto = IPPROTO_TCP;
		end->match.ipproto = IPPROTO_UDP;
	}
	else
		return FALSE;
	...
	/* Parse description */ // Note that it is used for block mode now
	if (desc) {
...
set_filter_client(int which, const netconf_filter_t *start, const netconf_filter_t *end)
{
	...
	{
		if( end->match.ipproto == start->match.ipproto )
		cur = safe_snprintf(cur, &len, "tcp");
		else
			cur = safe_snprintf(cur, &len, "both");
	}
