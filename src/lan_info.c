		...
		while (fread(&lease, sizeof(lease), 1, fp)){ 
			...
			if (!expires){   
				...
				strcpy(expires_time,"expired");
					...
					if (expires > 60*60*24) {
						sprintf(expires_time+strlen(expires_time),"%ld days, ",expires / (60*60*
24));
						expires %= 60*60*24;
					}
					if (expires > 60*60) {
						sprintf(expires_time+strlen(expires_time),"%02ld:",expires / (60*60));	
// hours
						expires %= 60*60;
					}
					else{
					...
					if (expires > 60) {
						sprintf(expires_time+strlen(expires_time),"%02ld:",expires / 60);	// 
minutes
						expires %= 60;
					}
					else{
					...
					for (j=0 ; j<dhcp_use ; j++){
						...
						if(strcmp(buf1, lan_host_table[j].ipaddr) == 0){
							...
							if (strncmp(buf1, "eth0", 4) != 0){
								...
								lan_host_table[lan_host_num].expire = 0; /* follow tr-069, if static 
ip, then leasetime is 0 */
					...
					if (strncmp(buf1, "eth0", 4) != 0){
						...
						strcpy(lan_host_table[lan_host_num].sourcetype, "static");
						lan_host_table[lan_host_num].isactive = 1;
						lan_host_table[lan_host_num].expire = 0; /* follow tr-069, if static ip
, then leasetime is 0 */
...
int get_lan_rx_tx_status(struct net_device_stats *stats)
{
	int res;
	res = re865xIoctl("eth0", RTL865X_IOCTL_GET_LANSTATUS, (unsigned int)stats,0,
0);
	...
	if ((fp = fopen("/tmp/route", "r")) != NULL) { 
		...
		while(fgets(tmp, 150, fp) != NULL){	
			if (i > 1){
                ...
                sprintf(buf1, "%s", strtok(tmp, tok1));     /* destination 
IPaddress */
...
int ip_ping_diagnostic(int vlaue)
{
   ...
   system("rm -rf /var/run/ping.txt");    // modify by dungjiun to keep only 
the last record in ping.txt	
   ...
   if (nvram_get("ipping_host") == NULL){
	   ...
	   if (nvram_get("ipping_numberofrepetitions") != NULL && nvram_get("
ipping_datablocksizes") != NULL){
		    sprintf(cmd, "ping -f -c %s -s %s %s", nvram_get("
ipping_numberofrepetitions"), 
						nvram_get("ipping_datablocksizes"), nvram_get("ipping_host"));
	   }
	   else if (nvram_get("ipping_numberofrepetitions") == NULL && nvram_get("
ipping_datablocksizes") != NULL){
		   sprintf(cmd, "ping -f -s %s %s", nvram_get("ipping_datablocksizes"), 
nvram_get("ipping_host")); 
	   }
	   else if (nvram_get("ipping_numberofrepetitions") != NULL && nvram_get("
ipping_datablocksizes") == NULL){
		   sprintf(cmd, "ping -f -c %s %s", nvram_get("ipping_numberofrepetitions")
, nvram_get("ipping_host"));
	   }
	   else if (nvram_get("ipping_numberofrepetitions") == NULL && nvram_get("
ipping_datablocksizes") == NULL){
   ...
   if (fp == NULL){
       ...
       counter += 3; /* count PING xxx.xxx.xxx.xxx (xxx.xxx.xxx.xxx): xx data 
bytes
	   ...
	   while(fgets(tmp, 150, fp) != NULL){	
		   ...
		   if (counter < 1){
			   if (counter == 0){             /* at  x packets transmitted, x packets 
received, x% packet loss line */
				   strcpy(buf1, tmp);
			   ...
			   else if (counter == -1){		 /* at round-trip min/avg/max = 0.0/8.0/40.0 
ms */
				   strcpy(buf2, tmp);
